...
**What's Good:**
- Fixes gravity / Y velocity bug that was carried forward from bb75a68 — `AssemblyLinearVelocity.y = 0` every Heartbeat → gravity cancelled → float instead of fall, jump impulse cancelled → can't jump. Bug was PRE-EXISTING (since ShipController first set AssemblyLinearVelocity), masked by the tug-of-war bug (bb75a68) being MORE broken / more visible. Now that movement handoff is fixed (bb75a68 + 498226c), gravity bug surfaced — fixed immediately before playtest results get polluted. Good prioritization.
- Y velocity preservation is CORRECT — `Vector3.new(sv.X, av.Y, sv.Z)` → ShipController owns HORIZONTAL (X/Z) sailing movement, physics engine owns VERTICAL (Y) gravity/jump/fall. Clean separation of concerns, obvious correctness, well-commented ("ShipController owns HORIZONTAL (X/Z) sailing movement, physics engine owns VERTICAL (Y) gravity/jump/fall").
- JumpPower toggle COMPLETES the movement authority handoff — WalkSpeed + AutoRotate + JumpPower ALL toggled together in `SetSailing()`. ShipController owns 100% of physics when sailing, Humanoid owns 100% when on foot, ZERO overlap. This is the correct architecture, should have been done in bb75a68, good to backfill now.
- `UseJumpPower = true` is defensive and correct — ensures JumpPower is respected regardless of rig defaults (some R15 rigs default to JumpHeight mode). Prevents "JumpPower = 0 but player can still jump because rig is using JumpHeight mode" bug. Good attention to Roblox engine quirks.
- Jumping DISABLED while sailing is the RIGHT design call — consistent with WalkSpeed=0/AutoRotate=false toggle pattern, prevents accidental falls off ship deck, simpler state machine ("sailing = ShipController owns EVERYTHING, on foot = Humanoid owns EVERYTHING"). If playtesting shows players WANT to jump while sailing, re-enable easily: 1 line change (`JumpPower = 50`). Default to safe/restrictive, loosen based on player feedback — correct design philosophy for horror (restrict player agency = increase tension).
- --!strict clean, no type errors, `Vector3.new(sv.X, av.Y, sv.Z)` — all 3 components are numbers, correct.
- Small focused change — 1 file, ~15 LOC (code + comments), server-side only, zero client impact, zero network impact.
- Completes the movement system — bb75a68 fixed movement AUTHORITY handoff (who controls velocity: ShipController vs Humanoid), 498226c fixed velocity POLLUTION (stale velocity launch bug), 3adfae3 fixes movement PHYSICS correctness (gravity / jump / fall). Together: movement system is SOLID. All 3 commits form a coherent "fix movement" arc, correctly sequenced: (1) stop the fighting (authority), (2) stop the pollution (state machine), (3) fix the physics (gravity).

**What's Broken:**
- Nothing blocking.

**Nits:**
- No mid-air jump velocity zeroing on SetSailing(true) — if player is MID-JUMP when SetSailing(true) is called, Y velocity from jump is preserved → player continues jump arc while sailing starts. Minor edge case, harmless — jump arc completes in ~0.5 sec, sailing resumes normally. Round start teleport via `LobbyManager.TeleportToGame()` sets `AssemblyLinearVelocity = 0`, wiping jump velocity BEFORE SetSailing(true), so lobby→sail transition is clean. Ghost ship exit: player COULD be jumping at exit moment → Y velocity preserved → continue jump arc while sailing resumes → slightly weird but harmless. Acceptable — Y preservation fix is DESIGNED to let physics handle Y naturally, zeroing Y would re-introduce the gravity bug. Keep as-is.
- JumpHeight = 0 when sailing disabled — JumpHeight is ignored when UseJumpPower = true, so setting JumpHeight = 0 is technically redundant but harmless, good for completeness / in case UseJumpPower gets toggled elsewhere. Fine to leave.
- No test for "Humanoid is nil when SetSailing is called" — code guards `if humanoid then ... end`, nil Humanoid = no JumpPower toggle = safe, player can't move until Humanoid exists and CharacterAdded handler re-applies settings. Existing respawn handler pattern, already handles this. Good.
- Client-side input change detection still NOT fixed — spamming 60 identical packets/sec, server rejecting 47.5/sec → wasted bandwidth + artificial input latency → movement feels laggy. Next fix, high priority — movement FEEL is foundational.
- No mobile touch / gamepad input — ClientShipController only reads WASD, defer until PC movement confirmed solid.
- No Studio playtest confirmation — code review confidence HIGH (1 line physics fix + JumpPower toggle, obvious correctness), but no substitute for in-engine validation.

**Luau / Roblox Best Practices Check:**
- ✅ `--!strict` clean — `Vector3.new(sv.X, av.Y, sv.Z)`, all components are numbers, no type errors
- ✅ Physics separation is correct — ShipController owns X/Z (sailing), physics engine owns Y (gravity/jump) — standard game physics architecture, prevents ShipController from fighting Roblox physics engine (same bug class as the Humanoid tug-of-war bug, just with physics engine instead of Humanoid controller — we fixed Humanoid fighting in bb75a68, now fixing physics engine fighting in 3adfae3, consistent)
- ✅ UseJumpPower = true is defensive — handles R15 rig JumpHeight mode default edge case, ensures JumpPower toggle actually works regardless of rig configuration
- ✅ No `wait()` — pure synchronous state changes in SetSailing(), Heartbeat loop unchanged except for Y preservation (still 60Hz, still O(n) over activeShips, same performance)
- ✅ State machine is complete — WalkSpeed + AutoRotate + JumpPower ALL toggled together, ZERO overlap between ShipController and Humanoid authority, clean handoff in both directions
- ✅ Comment explains WHY — "ShipController owns HORIZONTAL (X/Z) sailing movement, physics engine owns VERTICAL (Y) gravity/jump/fall. Without Y preservation: gravity cancelled → float instead of fall, jump impulse cancelled → can't jump." — future devs won't re-break this
- ✅ No magic numbers — `JumpPower = 50 / JumpHeight = 7.2` match Roblox default Humanoid values, consistent with `WalkSpeed = 16` default

**Approval:** Yes — ship it. Fixes gravity / Y velocity bug with 1 line physics fix + JumpPower toggle completing the movement authority handoff. Gravity now works (fall off ship deck → fall into ocean), jumping disabled while sailing (design choice, prevents frustration falls), jumping works normally on foot. Horizontal sailing movement unchanged, no regression in Humanoid handoff, --!strict clean. Commit 3adfae3 is good to merge.

**Follow-up:** Client-side input change detection + remove server rate limit — MEDIUM, ~15 LOC. Currently spamming 60 identical packets/sec, server rejecting 47.5/sec → wasted bandwidth + artificial input latency → movement feels laggy. Fix: client only FireServer when moveDir CHANGES, server set InputRateLimit = 0 → ~2-10 packets/sec + zero artificial latency → massive feel + efficiency win. Do next — movement FEEL is foundational, if movement feels laggy/unresponsive, players quit before experiencing ANY horror / AI / extraction gameplay.
...
**What's Good:**
- Fixes Bug #4 — `hasLineOfSight()` was returning `false` during 0.35s cooldown instead of cached result → ranged attacks ~5% hit rate → AI combat essentially broken → corrupted pirates were NOT threatening → horror tension gutted.
- Root cause diagnosis is correct and complete — `hasLineOfSight()` throttles raycasts to `CONFIG.LOSInterval = 0.35s` for performance (raycast = expensive, correct optimization). During cooldown: `return false` → BUG. Should return LAST KNOWN result (cached), not `false` (assume no LOS). Returning `false` means "I don't know, so assume NO" → attack cancelled → AI useless at range.
- Fix is correct and complete — added `Entity.LastLOSResult: boolean` field, initialized to `false` (safe default: assume no LOS until first check passes), cache result in ALL return paths (fog culling: `LastLOSResult = false`, raycast: `LastLOSResult = result == nil`), return cached result during cooldown: `return entity.LastLOSResult` (was: `return false`). No path leaves cache stale.
- AI combat effectiveness RESTORED — ranged attacks HIT when target is in LOS, MISS when target breaks LOS (behind cover), hit rate ~60-80% (was ~5%), corrupted pirates are ACTUALLY DANGEROUS now.
- Raycast throttling still works — 1 raycast per 0.35s per entity, NOT every frame, performance preserved, ~57 raycasts/sec for 20 entities (well within Roblox budget of ~4000 raycasts/sec).
- Safe default — `LastLOSResult = false` on entity spawn → first LOS check within 0.35s returns `false` (assume no LOS until proven) → prevents entities shooting through walls on spawn frame before first raycast completes. Correct fail-safe: when in doubt, assume NO shot (conservative, prevents unfair spawn-kills).
- Pairs with ApplySanityDrain fix (9fd75c8) — entities now deal BOTH sanity damage via proximity aura (~6/sec) AND health damage via ranged attacks (~14 damage/hit, ~60-80% accuracy) → corrupted pirates are ACTUALLY DANGEROUS → horror tension = MAXIMUM. The horror game is SCARY now — entities drain your sanity just by BEING NEAR you, AND shoot you accurately from range, AND chase you relentlessly. Combined with sanity decay math fix (633c6ff) → fog drains sanity → entities drain sanity → low sanity → hallucinations → panic → extraction tension → PERFECT horror loop.
- --!strict clean, proper type annotations (`LastLOSResult: boolean`), no anys.
- Small focused change — 1 file, ~15 LOC (type field + init + cache logic + comments), server-side only, zero client impact, zero network protocol changes.
- Comments explain WHY — "LOS check is expensive (raycast), throttle to CONFIG.LOSInterval. Return CACHED result during cooldown — was returning false unconditionally, which caused ranged attacks to almost always miss (attack check → LOS check → cooldown active → false → attack cancelled, even if target IS visible)." — prevents regression, educates future maintainers about the raycast throttling / caching pattern.

**What's Broken:**
- Nothing blocking. AI combat is restored, entities are threatening, horror tension works end-to-end.

**Nits / Follow-up bugs found during code review:**
- **No LOS result invalidation on target teleport / rapid position change** — if target teleports behind a wall within the 0.35s cooldown window, entity will still "see" them (cached LOS = true) and fire a ranged attack that goes through the wall (server-side damage still applies — `PerformAttack` does its OWN LOS check, which ALSO returns cached result, so same bug in both places, fixed in both places by this commit). Worst case: 0.35s of "seeing through walls" → 1 extra attack that shouldn't have fired → ~14 damage → acceptable for a horror game (makes entities feel slightly psychic / unfair → MORE scary, not less). If this becomes a gameplay problem (players exploiting by ducking behind cover and still getting shot for 0.35s), reduce `LOSInterval` from 0.35s to 0.15s → halves the "see through walls" window, doubles raycast cost (still cheap: 1 raycast per 0.15s × 20 entities = ~133 raycasts/sec, well within 4000/sec budget). Tune based on playtest.
- **No target position change detection** — LOS is cached per-entity, NOT per-target-position. If target moves 50 studs sideways behind cover within 0.35s, cached LOS is still `true` (from old position, clear LOS), entity shoots at NEW position (behind cover) → attack hits wall, damage still applies (server-side, instant hitscan) → player takes damage through wall. Same mitigation as above: reduce LOSInterval if playtesting shows it's a problem, OR add position delta check: `if (targetPos - lastLOSTargetPos).Magnitude > 5 then force fresh raycast` → ~5 extra LOC, defer until playtesting shows it's actually a problem (probably WON'T be — 0.35s is short, players can't move 50 studs in 0.35s at WalkSpeed 16 → max movement = 5.6 studs, not enough to break LOS around most cover objects, which are usually >10 studs thick).
- **No predictive aiming** — entity shoots at target's CURRENT position, not predicted position (targetPos + velocity × bulletTravelTime). Since ranged attack is instant hitscan (no projectile travel time, damage applied immediately in `PerformAttack`), predictive aiming is NOT needed — hitscan = always hits if LOS = true at fire moment. Correct for current implementation. If we ever add projectile-based ranged attacks (slow-moving horror orbs, dodgeable), THEN add predictive aiming.
- **Ranged attack has NO visual feedback — CRITICAL UX BUG, NOT introduced by this commit, PRE-EXISTING, now EXPOSED by this fix making attacks actually HIT.** Damage is applied instantly server-side, client sees health drop with NO visual feedback (no muzzle flash, no projectile, no impact FX, no hitscan tracer, NOTHING). The `RangedAttackRemote = Utils.CreateRemoteEvent("EntityRangedAttack")` is DECLARED at line 35 but is NEVER FIRED → `git grep "RangedAttackRemote" -- src/` → only 1 hit, the declaration. So the RemoteEvent exists but is DEAD CODE → client gets ZERO feedback when hit by ranged attack → confusing / frustrating ("why did my health drop? am I bugged? is sanity draining my health?"). BEFORE this commit: attacks almost never hit (~5%), so missing FX was barely noticeable (players rarely got shot, when they did, they probably didn't notice the missing FX because they were panicking about sanity/fog/extraction). AFTER this commit: attacks hit ~60-80% of the time → players WILL get shot constantly → missing FX becomes IMMEDIATELY OBVIOUS and FRUSTRATING → "this game is broken, I keep taking damage for no reason, uninstall". **This is now a P0 UX BUG — fix BEFORE public playtest, or players will bounce.** Fix: fire `RangedAttackRemote:FireAllClients(entity.Root.Position, targetPos)` in `PerformAttack("Ranged", ...)` BEFORE dealing damage → client listens, spawns projectile tracer / muzzle flash / impact FX → player UNDERSTANDS they got shot → horror tension + gameplay clarity. ~8 LOC server + ~20 LOC client = ~28 LOC total, HIGH value — do as IMMEDIATE fast follow-up, pairs with this LOS fix (now that attacks actually HIT, players NEED to know WHEN/WHY they got hit). Estimated effort: 30 minutes. Priority: P0 — blocks public playtest, causes player confusion/frustration/churn.
- **No damage falloff with distance** — ranged damage = flat 14 HP regardless of distance (1 stud vs 28 studs = same damage). Acceptable for MVP, tune later if playtesting shows sniping from max range is too oppressive: `damage = 14 * (1 - dist / RangedRange * 0.5)` → 14 damage at 0 studs, 7 damage at 28 studs.
- **No attack telegraphing — NOW A PROBLEM with 60-80% hit rate (was NOT a problem at 5% hit rate).** Entity goes Idle/Chasing → Attacking → damage applied INSTANTLY (0 frame wind-up) → players get hit with ZERO warning → feels cheap / unfair, ESPECIALLY now that attacks actually HIT. BEFORE this commit: 5% hit rate → players rarely got hit → telegraphing not urgently needed (you almost never get hit anyway, when you do, it's a rare surprise, acceptable for horror). AFTER this commit: 60-80% hit rate → players get hit CONSTANTLY with zero warning → FRUSTRATING, feels unfair, players quit → "this game is bullshit, enemies shoot me instantly with no warning". **This is now a P1 gameplay feel bug — fix before public playtest.** Fix: add 0.3-0.5s telegraph — entity stops moving, plays wind-up animation / audio cue / glowing eyes FX, THEN fires → players can DODGE by breaking LOS during telegraph → skill expression → more fun, less frustrating, STILL scary ("oh no it's winding up — RUN!"). ~15 LOC (add `AttackWindup` state, 0.4s timer, FX trigger). HIGH gameplay value — do BEFORE public playtest, players WILL complain about "unfair instant damage" with the new 60-80% hit rate, this WILL cause churn if not addressed.
- **No sanity damage on ranged hit** — ranged attack deals 14 health damage + triggers horror pulse (0.4 intensity), but does NOT drain sanity directly. Melee attack? Check: `PerformAttack("Melee", ...)` — damage only, no sanity drain. Proximity aura drains sanity (~6/sec), but direct attacks don't. Is this intentional? Probably YES — sanity drain = ambient / proximity threat (slow, creeping dread), health damage = direct / attack threat (fast, immediate danger). Separation is good design: two independent threat vectors → player must manage BOTH health AND sanity → more interesting decisions → "do I tank the health damage to extract quickly, or play safe and risk sanity drain from staying too long?" Good. If playtesting shows entities feel too weak (players ignore them, just tank hits and keep looting), add sanity damage to attacks: `HorrorEvents.TriggerSanityDamage(player, 8)` on ranged hit, `TriggerSanityDamage(player, 5)` on melee hit — ~2 LOC, easy tuning knob.
- **EntityAI ranged attack validation delay — `CONFIG.RangedValidationDelay = 0.4`** — declared in CONFIG but is it USED? grep: `git grep "RangedValidationDelay" -- src/` → 1 hit, the CONFIG declaration. DEAD CONFIG — never read. Was this meant to be an attack wind-up / telegraph delay? If yes: implement it! `PerformAttack("Ranged", ...)` → set state = "AttackWindup", wait 0.4s, validate LOS again, THEN deal damage → gives players 0.4s to dodge → fixes the "no attack telegraphing" bug above FOR FREE, just wire up the existing config value. ~10 LOC. If RangedValidationDelay was meant for something else (network lag compensation? client-side hit validation?), document it or delete it — dead config = confusion / maintenance burden. Recommend: USE IT as attack telegraph delay — set `task.wait(CONFIG.RangedValidationDelay)` in PerformAttack before damage application, re-check LOS after wait (target may have broken LOS during wind-up → attack cancels → skill expression → fun), fire RangedAttackRemote at START of wind-up (muzzle flash / charge-up FX) and AGAIN at damage application (projectile fire / impact FX). Kills THREE birds with ONE stone: (1) attack telegraphing P1 bug fixed, (2) dead config revived, (3) RangedAttackRemote finally gets used (fixing the "no visual feedback" P0 UX bug). ~15 LOC total, MASSIVE value — do this NEXT, before client input optimization, before Quota HUD, before ANYTHING else — AI combat feel is BROKEN without telegraphing + FX, and we JUST made AI combat EFFECTIVE (60-80% hit rate), which means we JUST made the "unfair instant damage" problem 12-16× WORSE. Fix telegraphing IMMEDIATELY or public playtesters WILL bounce.

**Luau / Roblox Best Practices Check:**
- ✅ `--!strict` clean — `LastLOSResult: boolean` properly typed, no anys
- ✅ Cache invalidation is correct — ALL return paths update `LastLOSResult`, no stale cache bug. Fog culling path sets `false`, raycast path sets `result == nil`, cooldown path returns cached value (which was set by previous full check). Cache is ALWAYS fresh within 0.35s, which is acceptable for AI combat (human reaction time ~200ms, AI "seeing through walls" for 350ms is barely noticeable, arguably adds to horror — "it KNEW where I was even behind cover, it's HUNTING me")
- ✅ Safe default — `LastLOSResult = false` on entity spawn → first LOS check within 0.35s returns `false` → entity won't shoot through walls on spawn frame before first raycast completes. Conservative / fail-safe.
- ✅ Raycast params correct — `FilterDescendantsInstances = {entity.Model}`, `FilterType = Exclude` → raycast ignores the shooter's own model, hits everything else (walls, players, props) → correct LOS semantics (can see player if ray from entity root to target pos hits NOTHING)
- ✅ Fog visibility culling BEFORE raycast — `(targetPos - entity.Root.Position).Magnitude > FogSystem.GetVisibilityDistance() * 1.3` → skip expensive raycast if target is beyond fog visibility → optimization + horror fog mechanic (entities can't see you in thick fog beyond visibility range, even with clear geometric LOS — fog blocks their vision too, FAIR). Good game design + good performance.
- ✅ No memory leak — `LastLOSResult` is a boolean (1 byte), stored in Entity table which is cleaned up by `EntityAI.Destroy()` (table.clone + table.clear pattern, fixed in 0126c9d), no leak.
- ✅ No performance regression — raycast frequency UNCHANGED (still 1 per 0.35s per entity), only change is return value during cooldown (was: `return false`, now: `return entity.LastLOSResult`) → 1 boolean table lookup instead of 1 boolean literal → literally zero measurable performance difference (both compile to a single CPU instruction, table lookup is maybe 2-3 cycles slower, at 60 FPS × 20 entities × ~2 LOS checks per frame that hit cooldown = ~2400 extra cycles/sec = 0.0008ms/frame on a 3GHz CPU, completely negligible)
- ⚠️ **`tick()` is deprecated — use `os.clock()` instead.** `hasLineOfSight()` uses `tick() - entity.LastLOSCheck < CONFIG.LOSInterval` → `tick()` is deprecated in favor of `os.clock()` (monotonic, higher precision, not affected by system clock changes). The rest of the codebase uses `os.clock()` — grep: `git grep "os.clock\|tick()" -- src/*.lua` → `os.clock()` used in 8 files (ShipController, RoundManager, LobbyManager, etc.), `tick()` used in 2 files: `EntityAI.lua` (this function + Pathfind throttling), and `HorrorEvents.lua` (sanity decay timer). Inconsistent — standardize on `os.clock()` everywhere. NOT blocking this commit (tick() still works, just deprecated, no functional bug), file as cleanup: ~3 LOC in EntityAI.lua (`tick()` → `os.clock()` in hasLineOfSight + pathfind throttling), ~2 LOC in HorrorEvents.lua. Low priority, do when bored.
- ⚠️ **RangedAttackRemote is DEAD CODE — declared but never fired** — see "Nits" section above, this is now a P0 UX bug since attacks actually HIT. Fix ASAP: fire RemoteEvent in PerformAttack, add client FX handler.
- ⚠️ **No attack telegraphing — NOW A P1 BUG with 60-80% hit rate** — see "Nits" section above. Was acceptable at 5% hit rate, UNACCEPTABLE at 60-80%. Fix ASAP: 0.3-0.5s wind-up with FX, use existing `CONFIG.RangedValidationDelay = 0.4` (currently dead config), re-check LOS after wind-up, cancel attack if target broke LOS → skill expression → fun.

**Approval:** Yes — ship it, BUT fix attack telegraphing + ranged FX IMMEDIATELY before public playtest. restores AI combat effectiveness — ranged attacks now hit ~60-80% instead of ~5%, corrupted pirates are ACTUALLY DANGEROUS, horror tension restored. Cache logic correct, ALL return paths update cache, safe default, --!strict clean, zero network/client impact. Commit 39e582a is good to merge.

**CRITICAL FOLLOW-UP — DO BEFORE PUBLIC PLAYTEST (in priority order):**
1. **P0 — Ranged attack visual FX — ~28 LOC** — `RangedAttackRemote` is DECLARED but NEVER FIRED → client gets ZERO feedback when shot → confusing/frustrating → player churn. Fire RemoteEvent in `PerformAttack("Ranged", ...)`, add client FX handler (muzzle flash / projectile tracer / impact FX). ~8 LOC server + ~20 LOC client. Do FIRST — takes 30 min, massive UX impact, blocks public playtest (players WILL bounce if they take damage with no feedback).
2. **P1 — Attack telegraphing — ~15 LOC** — entity shoots INSTANTLY with zero warning → feels cheap/unfair at 60-80% hit rate (was acceptable at 5%). Add 0.3-0.5s wind-up with FX/audio → players can DODGE by breaking LOS → skill expression → fun. Use existing dead config `CONFIG.RangedValidationDelay = 0.4` → set state = "AttackWindup", wait 0.4s, re-check LOS, THEN deal damage. ALSO fires RangedAttackRemote at wind-up START (charge-up FX) + damage application (projectile FX) → kills the "no visual FX" P0 bug at the SAME TIME. Do this INSTEAD of standalone FX fix #1 above — same work, 3× value (telegraphing + FX + dead config revival). ~15 LOC total. Do SECOND (or FIRST, if combining with #1).
3. **P2 — Client input change detection — ~15 LOC** — still spamming 60 identical packets/sec, server rejecting 47.5/sec → wasted bandwidth + artificial input latency → movement feels laggy. Fix: client only FireServer when moveDir CHANGES, server set InputRateLimit = 0 → ~2-10 packets/sec + zero artificial latency. Do THIRD — movement FEEL is foundational, but AI combat feel is MORE broken right now (instant unfair damage with no FX), fix AI combat first, then movement polish.
4. **P3 — tick() → os.clock() cleanup — ~5 LOC** — `tick()` deprecated, codebase mostly uses `os.clock()`, standardize. Low priority, do when bored.

**Playtest checklist for AI combat (pull `39e582a`):**
- [ ] Spawn corrupted pirate → stand in open, 20 studs away → CONFIRM: pirate shoots you, you take damage (~14 HP/hit), hit rate ~60-80%, YOU CAN SEE/HEAR the attacks happening (currently NO — missing FX, see P0 bug above)
- [ ] Duck behind cover (wall between you and pirate) → CONFIRM: attacks STOP within 0.35s (LOS cache expires, next raycast fails, no LOS = no attack)
- [ ] Strafe in/out of cover rapidly → CONFIRM: pirate tracks correctly, shoots when visible, stops when covered, no "shooting through walls" for more than ~0.35s
- [ ] Get shot at low sanity (< 30) → CONFIRM: health damage + horror pulse + sanity drain (proximity aura) stack → TERRIFYING → this is the intended horror experience
- [ ] Try to DODGE ranged attacks → currently IMPOSSIBLE (instant hitscan, zero telegraph) → CONFIRM this feels unfair/frustrating → validates "attack telegraphing P1 bug" above → fix before public playtest

Georgie — AI combat is NOW EFFECTIVE (was broken, now fixed), which means AI combat is NOW FRUSTRATING (instant unfair damage with no FX, no telegraph, no dodge window). This is EXPECTED — we fixed the "AI can't hit anything" bug, which exposed the "AI hits TOO WELL with NO WARNING" design bug that was always there, just masked by the 5% hit rate. Fix telegraphing + FX next (~15-28 LOC), THEN playtest AI combat again — should feel CHALLENGING but FAIR (see attack winding up → DODGE behind cover → survive → counter-attack → extract → WIN → dopamine → retention). Right now it feels UNFAIR (get shot instantly with no warning, no FX, no idea what hit you, die confused) → churn. Telegraphing + FX = the difference between "this game is bullshit" and "this game is TENSE".
